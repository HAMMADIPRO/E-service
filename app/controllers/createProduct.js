		...
		productModel.find({}).populate("owner", "firstName").exec(function(err,product){
			if(err){
				...
				var myResponse = responseGenerator.generate(
					false,"Service got successfully",200,product);
		           			...
		           			.exec(function(err,popProduct){
		           				...
		           				if(err){
		            				...
		            				newProduct.owner = popProduct.owner;
				           			...
				           			var myResponse = responseGenerator.generate(
				           				false,"Service created successfull",200,newProduct);
   	...
   	productRouter.get('/:id',auth.checkLogin,function(req,res){
		...
		.exec(function(err,product){
			if(err){
				...
				var myResponse = responseGenerator.generate(
					false,"Service found successfully",200,product);
			...
			if(arg1.owner.firstName == arg2.firstName){
				...
				productModel.remove({"_id":req.params.id},function(err,product){
					...
					if(err){
						...
						var myResponse = responseGenerator.generate(
							false,"Service Deleted successfully",200,forInfo.authCheck);
		  		...
		  		var myResponse = responseGenerator.generate(
		  			false,"You don't have the permission to delete other's service",200,
		  			forInfo.authCheck);
			...
			checkAuthorityAndDelete
				...
				if(err){
					...
					if(forInfo.authCheck == true){
						console.log("Inside waterfall will delete from My Services")
			...
			if(arg1.owner.firstName == arg2.firstName){
				...
				productModel.findByIdAndUpdate({"_id":req.params.id},update,{new:true},
					...
					if(err){
						...
						var myResponse = responseGenerator.generate(
							false,"Services Edited successfully",200,forInfo.authEditCheck);
	  			...
	  			var myResponse = responseGenerator.generate(
	  				false,"You don't have the permission to EDIT other's Service",200,

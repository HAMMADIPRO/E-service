	...
	cartRouter.get('/all',auth.checkLogin,function(req,res){
		...
		userModel.find({"_id":req.session.user._id},
			...
			if(err){
				...
				var myResponse = responseGenerator.generate(false,"No Services added to cart",200,ourInfo);
                res.send(myResponse);
			}
		...
		userModel.find({"_id":req.session.user._id},function(err,user){
				...
				if(toCheckCart == true && alertMessage==true){
						...
						var myResponse = responseGenerator.generate(
							false,"Service already present in cart !!!",200,true);
					     res.send(myResponse);
				}
					...
					var addToCart = function(arg1,callback){
						...
						if(itemCount == null){
							console.log("Add to My Services inside ran");
					...
					async.waterfall([
						...
						addToCart],function(err,result){
							...
							if(err){
								...
								var myResponse = responseGenerator.generate(
									false,"Add to My Services !!!",200,req.session.user);
					                res.send(myResponse);
			            	}
					})
				}
	...
	cartRouter.post('/delete/:id',auth.checkLogin,function(req,res){
		...
		if(req.body.toReduce == true){
				...
				userModel.update({"_id":req.session.user._id},
					{$pull:{cart:{"produtId":req.params.id}}},function(err,user){
					...
					if(err){
							...
							var myResponse = responseGenerator.generate(
								false,"Service removed from My Services!",200,ourInfo);
							res.send(myResponse);
					}
				})
		}
